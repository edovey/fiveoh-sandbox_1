        ...
        private BDHtmlPage generatePageForAntibioticsClinicalGuidelinesSpectrum(Entities pContext, BDNode pNode)
{
        ...
        private BDHtmlPage generatePageForAntibioticsDosingAndDailyCosts(Entities pContext, BDNode pNode)
        {
            ...
            if (antimicrobials.Count > 0)
            {
                ...
                {
                    string amHtml = buildNodePropertyHTML(pContext, antimicrobial, antimicrobial.Name, BDNode.PROPERTYNAME_NAME, false, footnotes, objectsOnPage);
        ...
        private BDHtmlPage generatePageForAntibioticsCSFPenetration(Entities pContext, BDNode pNode)
        {
            ...
            foreach (IBDNode child in childNodes)
            {
                if (child.LayoutVariant == BDConstants.LayoutVariantType.Antibiotics_CSFPenetration)
                {
                    ...
                    {
                        bodyHTML.Append(buildNodePropertyHTML(pContext, category, category.Name, BDNode.PROPERTYNAME_NAME, false, footnotesOnPage, objectsOnPage));
                        ...
                        if (subcategories.Count > 0)
                        {
                            ...
                            foreach (IBDNode column in subcategories)
                            {
                                ...
                                if (columnDetail.Count > 0)
                                {
                                    ...
                                    {
                                        string antimicrobialHTML = buildNodePropertyHTML(pContext, antimicrobial, antimicrobial.Name, BDNode.PROPERTYNAME_NAME, false, footnotesOnPage, objectsOnPage);
            ...
            switch (pNode.LayoutVariant)
            {
                ...
                case BDConstants.LayoutVariantType.TreatmentRecommendation01_Gastroenteritis_CultureDirected:
                    foreach (IBDNode entry in childNodes)
                    {
                        ...
                        {
                            case BDConstants.BDNodeType.BDPathogenGroup:
                                bodyHTML.Append(BuildBDPathogenGroupHtml(pContext, entry, footnotesOnPage, objectsOnPage, 3));
                                break;
                            case BDConstants.BDNodeType.BDTopic:
                                // describe the topic
                                bodyHTML.Append(buildNodeWithReferenceAndOverviewHTML(pContext, entry, "h3", footnotesOnPage, objectsOnPage));
                ...
                default:
                    ...
                    foreach (IBDNode pathogenGroup in childNodes)
                        bodyHTML.Append(BuildBDPathogenGroupHtml(pContext, pathogenGroup, footnotesOnPage, objectsOnPage, 3));
                        //bodyHTML.Append(buildEmpiricTherapyHTML(pContext, pathogenGroup as BDNode, footnotesOnPage, objectsOnPage));
                    break;
            }
        ...
        public BDHtmlPage GenerateBDHtmlPage(Entities pContext, IBDNode pNode)
        {
            ...
            switch (pNode.NodeType)
            {
                ...
                case BDConstants.BDNodeType.BDPathogen:
                    ...
                    break;
                case BDConstants.BDNodeType.BDPathogenGroup:
                    bodyHTML.Append(BuildBDPathogenGroupHtml(pContext, pNode, footnotes, objectsOnPage, 1));
                    break;
        ...
        private BDHtmlPage generatePageForEmpiricTherapyOfParasitic(Entities pContext, BDNode pNode)
        {
            ...
            foreach (IBDNode childNode in childNodes) 
            {
                ...
                {
                    bodyHTML.Append(buildNodePropertyHTML(pContext, childNode, childNode.Name, BDNode.PROPERTYNAME_NAME, false, footnotesOnPage, objectsOnPage));
        ...
        private BDHtmlPage generatePageForEmpiricTherapyOfCultureDirected(Entities pContext, BDNode pNode)
        {
            ...
            foreach (IBDNode resistance in resistances)
            {
                ...
                {
                    bodyHTML.Append(buildNodePropertyHTML(pContext, therapyGroup, therapyGroup.Name, BDTherapyGroup.PROPERTYNAME_NAME, false, footnotesOnPage, objectsOnPage));
           ...
           foreach(IBDNode child in childNodes)
            {
                ...
                bodyHTML.Append(buildNodePropertyHTML(pContext, child, child.Uuid, child.Name, BDNode.PROPERTYNAME_NAME, false, "h3", footnotesOnPage, objectsOnPage));
                    ...
                    if (l2Child.NodeType == BDConstants.BDNodeType.BDCombinedEntry)
                    {
                            ...
                            l2Child.LayoutVariant == BDConstants.LayoutVariantType.Prophylaxis_Communicable_Influenza_Oseltamivir_Creatinine)
                        {
                            tableHTML.AppendFormat("<tr><td rowspan=4>{0}</td><td>{1}</td><td>{2}</td></tr>",
                                buildNodePropertyHTML(pContext, cEntry, cEntry.Name, BDCombinedEntry.PROPERTYNAME_NAME, false, footnotesOnPage, objectsOnPage),
                                buildNodePropertyHTML(pContext, cEntry, cEntry.entryTitle01, BDCombinedEntry.PROPERTYNAME_ENTRYTITLE01, false, footnotesOnPage, objectsOnPage),
                                buildNodePropertyHTML(pContext, cEntry, cEntry.entryDetail01, BDCombinedEntry.PROPERTYNAME_ENTRY01, false, footnotesOnPage, objectsOnPage));
                            if (cEntry.entryDetail02 != null && cEntry.entryTitle02 != null)
                            {
                                tableHTML.AppendFormat("<tr><td>{0}</td><td>{1}</td></tr>",
                                    buildNodePropertyHTML(pContext, cEntry, cEntry.entryTitle02, BDCombinedEntry.PROPERTYNAME_ENTRYTITLE02, false, footnotesOnPage, objectsOnPage),
                                    buildNodePropertyHTML(pContext, cEntry, cEntry.entryDetail02, BDCombinedEntry.PROPERTYNAME_ENTRY02, false, footnotesOnPage, objectsOnPage));
                            }
                            if (cEntry.entryDetail03 != null && cEntry.entryTitle03 != null)
                            {
                                tableHTML.AppendFormat("<tr><td>{0}</td><td>{1}</td></tr>",
                                    buildNodePropertyHTML(pContext, cEntry, cEntry.entryTitle03, BDCombinedEntry.PROPERTYNAME_ENTRYTITLE03, false, footnotesOnPage, objectsOnPage),
                                    buildNodePropertyHTML(pContext, cEntry, cEntry.entryDetail03, BDCombinedEntry.PROPERTYNAME_ENTRY03, false, footnotesOnPage, objectsOnPage));
                            }
                            if (cEntry.entryTitle04 != null && cEntry.entryDetail04 != null)
                            {
                                tableHTML.AppendFormat("<tr><td>{0}</td><td>{1}</td></tr>",
                                    buildNodePropertyHTML(pContext, cEntry, cEntry.entryTitle04, BDCombinedEntry.PROPERTYNAME_ENTRYTITLE04, false, footnotesOnPage, objectsOnPage),
                                    buildNodePropertyHTML(pContext, cEntry, cEntry.entryDetail04, BDCombinedEntry.PROPERTYNAME_ENTRY04, false, footnotesOnPage, objectsOnPage));
                            }
                        }
                        ...
                        else
                        {
                            ...
                            string groupTitleHtml = buildNodePropertyHTML(pContext, cEntry, cEntry.groupTitle, BDCombinedEntry.PROPERTYNAME_GROUPTITLE, false, footnotesOnPage, objectsOnPage);
                        ...
                        foreach (IBDNode tChild in topicChildren)
                        {
                            if (tChild.NodeType == BDConstants.BDNodeType.BDConfiguredEntry)
                            {
                                ...
                                tableHTML.AppendFormat("<tr><td>{0}</td><td><{1}</td><td>{2}</td><td>{3}</td></tr>", 
                                    buildNodePropertyHTML(pContext, cEntry, cEntry.Name, BDConfiguredEntry.PROPERTYNAME_NAME, false, footnotesOnPage, objectsOnPage),
                                    buildNodePropertyHTML(pContext, cEntry, cEntry.field01, BDConfiguredEntry.PROPERTYNAME_FIELD01, false, footnotesOnPage, objectsOnPage),
                                    buildNodePropertyHTML(pContext, cEntry, cEntry.field02, BDConfiguredEntry.PROPERTYNAME_FIELD02, false, footnotesOnPage, objectsOnPage),
                                    buildNodePropertyHTML(pContext, cEntry, cEntry.field03, BDConfiguredEntry.PROPERTYNAME_FIELD03, false, footnotesOnPage, objectsOnPage));
                            }
            ...
            foreach(IBDNode table in childNodes)
            {
                ...
                foreach (IBDNode therapyGroup in tableChildren)
                {
                    ...
                    foreach (IBDNode therapy in therapies)
                    {
                        ...
                        string marker = buildFooterMarkerForList(retrieveNotesForParentAndPropertyOfLinkedNoteType(pContext, t.Uuid, BDTherapy.PROPERTYNAME_DOSAGE, BDConstants.LinkedNoteType.MarkedComment), true, footnotesOnPage, objectsOnPage);
                        ...
                        if (tGroups.Count > 0)
                        {
                            ...
                            foreach (IBDNode therapyGroup in tGroups)
                            {
                                ...
                                foreach (IBDNode t in therapies)
                                {
                                    ...
                                    {
                                        adultDosageHTML.AppendFormat("<li>{0}", buildNodePropertyHTML(pContext, therapy, previousTherapyId, previousTherapyName, BDTherapy.PROPERTYNAME_THERAPY, false, footnotesOnPage, objectsOnPage));
                                        pedsDosageHTML.AppendFormat("<li>{0}", buildNodePropertyHTML(pContext, therapy, previousTherapyId, previousTherapyName, BDTherapy.PROPERTYNAME_THERAPY, false, footnotesOnPage, objectsOnPage));
                                    }
                                    else
                                    {
                                        adultDosageHTML.AppendFormat("<li>{0}", buildNodePropertyHTML(pContext, therapy, therapy.Uuid, therapy.Name, BDTherapy.PROPERTYNAME_THERAPY, false, footnotesOnPage, objectsOnPage));
                                        pedsDosageHTML.AppendFormat("<li>{0}", buildNodePropertyHTML(pContext, therapy, therapy.Uuid, therapy.Name, BDTherapy.PROPERTYNAME_THERAPY, false, footnotesOnPage, objectsOnPage));
                                    }
                                    // Dosage - adult dose
                                    if (therapy.dosageSameAsPrevious.Value == true)
                                        adultDosageHTML.Append(buildNodePropertyHTML(pContext, therapy, previousTherapyId, previousTherapyDosage, BDTherapy.PROPERTYNAME_DOSAGE, false, footnotesOnPage, objectsOnPage));
                                    else
                                        adultDosageHTML.Append(buildNodePropertyHTML(pContext, therapy, therapy.Uuid, therapy.dosage, BDTherapy.PROPERTYNAME_DOSAGE, false, footnotesOnPage, objectsOnPage));

                                    // Dosage 1 - Paediatric dose
                                    if (therapy.dosage1SameAsPrevious.Value == true)
                                        pedsDosageHTML.Append(buildNodePropertyHTML(pContext, therapy, previousTherapyId, previousTherapyDosage1, BDTherapy.PROPERTYNAME_DOSAGE_1, false, footnotesOnPage, objectsOnPage));
                                    else
                                        pedsDosageHTML.Append(buildNodePropertyHTML(pContext, therapy, therapy.Uuid, therapy.dosage1, BDTherapy.PROPERTYNAME_DOSAGE_1, false, footnotesOnPage, objectsOnPage));
                ...
                foreach (IBDNode antimicrobial in antimicrobials)
                {
                    ...
                    foreach (IBDNode amRisk in amRisks)
                    {
                        ...
                        if (risk.riskFactor.Length > 0)
                            antimicrobialHTMLBody.AppendFormat("<b>{0}</b>: {1}", c1Html, buildNodePropertyHTML(pContext, risk, risk.riskFactor, BDAntimicrobialRisk.PROPERTYNAME_PREGNANCYRISK, false, footnotesOnPage, objectsOnPage));
                        if (risk.recommendations.Length > 0)
                            antimicrobialHTMLBody.AppendFormat("<p><b>{0}</b><br>{1}</p>", c2Html, buildNodePropertyHTML(pContext, risk, risk.recommendations, BDAntimicrobialRisk.PROPERTYNAME_RECOMMENDATION, false, footnotesOnPage, objectsOnPage));
        ...
        private BDHtmlPage generatePageForOrganismGramStain(Entities pContext, IBDNode pNode)
        {
            ...
            if (childNodes.Count > 0) // subcategory - column 1 values
            {
                ...
                foreach (IBDNode child in childNodes)
                {
                    ...
                    {
                        mString.AppendFormat("{0}<br>", buildNodePropertyHTML(pContext, microorganism, microorganism.Name, BDNode.PROPERTYNAME_NAME, false, footnotesOnPage, objectsOnPage));
                    }
                    string scString = buildNodePropertyHTML(pContext, subcategory, subcategory.Name, BDNode.PROPERTYNAME_NAME, false, footnotesOnPage, objectsOnPage);
        ...
        private BDHtmlPage generatePageForMicrobiologyOrganisms(Entities pContext, IBDNode pNode)
        {
            ...
            {
                // microorganism group
                bodyHTML.Append(buildNodePropertyHTML(pContext, subcategory, subcategory.Uuid, subcategory.Name, BDNode.PROPERTYNAME_NAME, false, "h2", footnotesOnPage, objectsOnPage));


                List<IBDNode> mGroups = BDFabrik.GetChildrenForParent(pContext, subcategory);
                foreach (IBDNode mGroup in mGroups)
                {
                    bodyHTML.Append(buildNodePropertyHTML(pContext, mGroup, mGroup.Uuid, mGroup.Name, BDNode.PROPERTYNAME_NAME, false, "h4", footnotesOnPage, objectsOnPage));

                    List<IBDNode> microorganisms = BDFabrik.GetChildrenForParent(pContext, mGroup);
                    foreach (IBDNode microorganism in microorganisms)
                        bodyHTML.AppendFormat("{0}<br>", buildNodePropertyHTML(pContext, microorganism, microorganism.Name, BDNode.PROPERTYNAME_NAME, false, footnotesOnPage, objectsOnPage));
                }
            }
            return writeBDHtmlPage(pContext, pNode, bodyHTML, BDConstants.BDHtmlPageType.Data, footnotesOnPage, objectsOnPage);
        }
            ...
            //ks: Consider refactoring to use buildBDPathogenGroupHtml
             ...
             StringBuilder bodyHtml = new StringBuilder();
                ...
                foreach (BDTherapyGroup tGroup in therapyGroups)
                {
                    ...
                    if (null != group)
                    {
                        bodyHtml.Append(buildNodePropertyHTML(pContext, group, group.Name, BDTherapyGroup.PROPERTYNAME_NAME, false, pFootnotes, pObjectsOnPage));
            ...
            if (therapies.Count > 0)
            {
                ...
                switch (pTherapyGroup.LayoutVariant)
                {
                    case BDConstants.LayoutVariantType.TreatmentRecommendation01:
                        ...
                        if (therapyDosageTitleHtml.Length == 0) therapyDosageTitleHtml = "Recommended<br>Dose";
                        if (therapyDurationTitleHtml.Length == 0) therapyDurationTitleHtml = "Recommended<br>Duration";
                    ...
                    case BDConstants.LayoutVariantType.TreatmentRecommendation18_CultureProvenEndocarditis_Paediatrics:
                        if (!therapiesHaveDosage && !therapiesHaveDuration)
                            ...
                            therapyGroupHtml.Append(@"<tr><th>Antibiotics</th>");
                        therapyGroupHtml.Append(therapiesHaveDosage ? @"<th>Dose</th>" : @"<th></th>");
                        therapyGroupHtml.Append(therapiesHaveDuration ? @"<th colspan=2>Recommended<br>Duration</th>" : @"<th></th>");
        ...
        /// <param name="pObjectsOnPage"></param>
        /// <returns></returns>
        public string BuildBDPathogenGroupHtml(Entities pContext, IBDNode pNode, List<BDLinkedNote> pFootnotes, List<Guid> pObjectsOnPage, int pLevel)
        {
            ...
            if (null != pNode && pNode.NodeType == BDConstants.BDNodeType.BDPathogenGroup)
            {
                ...
                switch (pNode.LayoutVariant)
                {
                    case BDConstants.LayoutVariantType.TreatmentRecommendation05_CultureProvenPeritonitis:
                        ...
                        foreach (IBDNode child in children)
                        {
                            html.Append(buildNodePropertyHTML(pContext, child, child.Name, BDTherapyGroup.PROPERTYNAME_NAME, false, pFootnotes, pObjectsOnPage));
                            // custom-built - Therapy Group has 2 dosages and a custom header
                            ...
                            if (therapies.Count > 0)
                            {
                                ...
                                if (!therapiesHaveDosage && !therapiesHaveDuration)
                                    html.Append(@"<tr><th>Recommended Empiric Therapy</th>");
                                else
                                    html.Append(@"<tr><th>Recommended<br>Empiric<br>Therapy</th>");

                                if (therapiesHaveDosage)
                                {
                                    html.Append(@"<th>Intermittent Dosing<br>Dwell time at least 6h</th>");
                                    html.Append(@"<th>Continuous dosing<br>(per L bag)</th>");
                                }
                                else
                                    html.Append(@"<th />");

                                if (therapiesHaveDuration)
                                    html.Append(@"<th>Recommended<br>Duration</th>");
                                else
                                    html.Append(@"<th />");
                    ...
                    default:
                        ...
                        if (children.Count > 0)
                        {
                            html.Append(string.Format("<{0}>Usual Pathogens</{0}>", HtmlHeaderTagLevelString(pLevel + 1)));
                        }
                        ...
                        foreach (IBDNode child in children)
                        {
                            switch (child.NodeType)
                            {
                                ...
                                case BDConstants.BDNodeType.BDTherapyGroup:
                                    ...
                                    {
                                        html.AppendFormat("{0}<br>", (BuildBDTherapyGroupHTML(pContext, therapyGroup, pFootnotes, pObjectsOnPage, pLevel + 1, null)));
                                    }
                                    break;
                            }
                        }
            ...
            if ((null != pNode) && (pNode.NodeType == BDConstants.BDNodeType.BDPathogen))
            {
                ...
                switch (pNode.LayoutVariant)
                {
                    ...
                    case BDConstants.LayoutVariantType.TreatmentRecommendation15_CultureProvenPneumonia:
                        ...
                        foreach (IBDNode child in children)
                        {
                            ...
                            foreach (IBDNode therapyGroupChild in therapyGroups)
                            {
                                ...
                                html.Append(buildNodePropertyHTML(pContext, therapyGroupChild, therapyGroupChild.Name, BDTherapyGroup.PROPERTYNAME_NAME, false, pFootnotes, pObjectsOnPage));
                    ...
                    case BDConstants.LayoutVariantType.TreatmentRecommendation09_Parasitic_II:
                        ...
                        // overview
                        string symptomsOverview = retrieveNoteTextForOverview(pContext, pNode.Uuid, pObjectsOnPage);
                        if (symptomsOverview.Length > EMPTY_PARAGRAPH)
                            html.AppendFormat(@"<u><b>Symptoms</b></u><br>{0}",symptomsOverview);
                        ...
                        foreach (IBDNode child in children) 
                        {
                            switch (child.NodeType)
                            {
                                case BDConstants.BDNodeType.BDPresentation:
                                    ...
                                    string presentationOverview = retrieveNoteTextForOverview(pContext, child.Uuid, pObjectsOnPage);
                                    if (presentationOverview.Length > EMPTY_PARAGRAPH)
                                        html.AppendFormat(@"<u><b>Comments</b></u><br>{0}", presentationOverview);
                                    break;
                                case BDConstants.BDNodeType.BDTherapyGroup:
                                    html.Append(buildNodePropertyHTML(pContext, child, child.Name, BDNode.PROPERTYNAME_NAME, false, pFootnotes, pObjectsOnPage));
                    ...
                    case BDConstants.LayoutVariantType.TreatmentRecommendation07_CultureProvenEndocarditis:
                        ...
                        foreach (IBDNode child in children)
                        {
                            ...
                            if (therapies.Count > 0)
                            {
                                ...
                                if (therapiesHaveDosage)
                                {
                                    html.Append(@"<th>Recommended<br>Dose</th>");
                                }
                                else
                                    html.Append(@"<th></th>");
                                if (therapiesHaveDuration)
                                    html.Append(@"<th colspan=2>Recommended<br>Duration</th>");
                                else
                                    html.Append(@"<th></th>");

                                html.Append(@"</tr>");
                                html.Append(@"<tr><th /><th /><th>Native</th><th>Prosthetic</th></tr>");
            ...
            if (null != pNode && pNode.NodeType == BDConstants.BDNodeType.BDAntimicrobial)
            {
                ...
                switch (pNode.LayoutVariant)
                {
                    ...
                    case BDConstants.LayoutVariantType.Antibiotics_DosingAndCosts_Paediatric:
                        ...
                        string amHtml = buildNodePropertyHTML(pContext, pNode, pNode.Name, BDNode.PROPERTYNAME_NAME, false, pFootnotes, pObjectsOnPage);
                    ...
                    case BDConstants.LayoutVariantType.Antibiotics_Dosing_RenalImpairment:
                        ...
                        string antimicrobialHtml = buildNodePropertyHTML(pContext, pNode, pNode.Name, BDNode.PROPERTYNAME_NAME, false, pFootnotes, pObjectsOnPage);
                        ...
                        foreach (IBDNode child in children)
                        {
                            switch (child.NodeType)
                            {
                                ...
                                case BDConstants.BDNodeType.BDDosageGroup:
                                    if (!isFirstChild) html.Append(rowStartTag);
                                    dosageGroupName = buildNodePropertyHTML(pContext, child, child.Uuid, child.Name, BDNode.PROPERTYNAME_NAME, false, "u", pFootnotes, pObjectsOnPage);
                ...
                switch (pNode.LayoutVariant)
                {
                    ...
                    case BDConstants.LayoutVariantType.Antibiotics_CSFPenetration_Dosages:
                        ...
                        if (null != metadataLayoutColumns)
                        {
                            ...
                            for (int i = 0; i < metadataLayoutColumns.Count; i++)
                                html.AppendFormat("<th>{0}</th>", buildHtmlForMetadataColumn(pContext, pNode, metadataLayoutColumns[i], BDConstants.BDNodeType.BDConfiguredEntry, BDConfiguredEntry.PropertyNameForIndex(i), pFootnotes, pObjectsOnPage));
                            html.Append("</tr>");
                    ...
                    case BDConstants.LayoutVariantType.Prophylaxis_FluidExposure_Followup_II:
                        ...
                        foreach (IBDNode entry in childNodes)
                        {
                            ...
                            html.AppendFormat(@"<table><tr colspan=3><th>RECIPIENT{0}</th></tr>", c1Html);
        ...
        public string BuildBDTableSectionHtml(Entities pContext, IBDNode pNode, List<BDLinkedNote> pFootnotes, List<Guid> pObjectsOnPage, int pLevel, int pColumnCount)
        {
            ...
            if ((null != pNode) && (pNode.NodeType == BDConstants.BDNodeType.BDTableSection))
            {
                ...
                html.AppendFormat(@"<tr><td colspan={0}>{1}</td></tr>", pColumnCount, pNode.Name);
                ...
                foreach (IBDNode child in children)
                {
                    switch (child.NodeType)
                    {
                        ...
                        case BDConstants.BDNodeType.BDTableSubsection:
                            html.AppendFormat(@"<tr><td colspan={0}><u>{1}</u></td></tr>", pColumnCount, child.Name);
            ...
            if ((null != pNode) && (pNode.NodeType == BDConstants.BDNodeType.BDConfiguredEntry))
            {
                ...
                if (null != configuredEntry)
                {
                    ...
                    if (null != metadataLayoutColumns)
                    {
                        ...
                        {
                            html.AppendFormat("<td>{0}</td>", buildNodePropertyHTML(pContext, configuredEntry, configuredEntry.PropertyValueForIndex(idx), BDConfiguredEntry.PropertyNameForIndex(idx), false, pFootnotes, pObjectsOnPage));
                        }
                    }
                    ...
                    switch (configuredEntry.LayoutVariant)
                    {
                        case BDConstants.LayoutVariantType.Prophylaxis_Communicable_Influenza_Amantadine_Renal:
                            html.AppendFormat("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td></tr>",
                                              ...
                                              buildNodePropertyHTML(pContext, configuredEntry, configuredEntry.field03, BDConfiguredEntry.PROPERTYNAME_FIELD03, false, pFootnotes, pObjectsOnPage));
                            break;
                ...
                switch (pNode.LayoutVariant)
                {
                    case BDConstants.LayoutVariantType.Dental_Prophylaxis_DrugRegimens:
                        foreach (IBDNode surgeryClassification in surgeryClassificationList) // SurgeryClassification
                        {
                            ...
                            List<IBDNode> therapyGroupChildren = BDFabrik.GetChildrenForParent(pContext, surgeryClassification);
                            if (therapyGroupChildren.Count > 0)
                            {
                                StringBuilder adultDosageHTML = new StringBuilder();
                                StringBuilder pedsDosageHTML = new StringBuilder();
                                html.Append(@"<table>");
                                html.AppendFormat(@"<tr><th>{0}</th><th>{1}</th></tr>", c2Html, c3Html);
                                foreach (IBDNode therapyGroup in therapyGroupChildren)
                                {
                                    if (therapyGroup.Name.Length > 0 && !therapyGroup.Name.Contains("New Therapy Group"))
                                    {
                                        adultDosageHTML.AppendFormat("<u>{0}</u><ul>", therapyGroup.Name);
                                    ...
                                    foreach (IBDNode t in therapies)
                                    {
                                        ...
                                        if (therapy.nameSameAsPrevious.Value == true)
                                        {
                                            adultDosageHTML.AppendFormat("<li>{0}", buildNodePropertyHTML(pContext, therapy, previousTherapyId, previousTherapyName, BDTherapy.PROPERTYNAME_THERAPY, false, pFootnotes, pObjectsOnPage));
                                            pedsDosageHTML.AppendFormat("<li>{0}", buildNodePropertyHTML(pContext, therapy, previousTherapyId, previousTherapyName, BDTherapy.PROPERTYNAME_THERAPY, false, pFootnotes, pObjectsOnPage));
                                        }
                                        else
                                        {
                                            adultDosageHTML.AppendFormat("<li>{0}", buildNodePropertyHTML(pContext, therapy, therapy.Uuid, therapy.Name, BDTherapy.PROPERTYNAME_THERAPY, false, pFootnotes, pObjectsOnPage));
                                            pedsDosageHTML.AppendFormat("<li>{0}", buildNodePropertyHTML(pContext, therapy, therapy.Uuid, therapy.Name, BDTherapy.PROPERTYNAME_THERAPY, false, pFootnotes, pObjectsOnPage));
                                        }
                                        // Dosage - adult dose
                                        if (therapy.dosageSameAsPrevious.Value == true)
                                            adultDosageHTML.Append(buildNodePropertyHTML(pContext, therapy, previousTherapyId, previousTherapyDosage, BDTherapy.PROPERTYNAME_DOSAGE, false, pFootnotes, pObjectsOnPage));
                                        else
                                            adultDosageHTML.Append(buildNodePropertyHTML(pContext, therapy, therapy.Uuid, therapy.dosage, BDTherapy.PROPERTYNAME_DOSAGE, false, pFootnotes, pObjectsOnPage));

                                        // Dosage 1 - Paediatric dose
                                        if (therapy.dosage1SameAsPrevious.Value == true)
                                            pedsDosageHTML.Append(buildNodePropertyHTML(pContext, therapy, previousTherapyId, previousTherapyDosage1, BDTherapy.PROPERTYNAME_DOSAGE_1, false, pFootnotes, pObjectsOnPage));
                                        else
                                            pedsDosageHTML.Append(buildNodePropertyHTML(pContext, therapy, therapy.Uuid, therapy.dosage1, BDTherapy.PROPERTYNAME_DOSAGE_1, false, pFootnotes, pObjectsOnPage));
            ...
            if ((null != pNode) && (pNode.NodeType == BDConstants.BDNodeType.BDCategory))
            {
                ...
                switch (pNode.LayoutVariant)
                {
                        ...
                        if (children.Count > 0)
                        {
                            ...
                            foreach (IBDNode child in children)
                            {
                                ...
                                if (columnDetail.Count > 0)
                                {
                                    ...
                                    foreach (IBDNode antimicrobial in columnDetail)
                                    {
                                        string antimicrobialHTML = buildNodePropertyHTML(pContext, antimicrobial, antimicrobial.Name, BDNode.PROPERTYNAME_NAME, false, pFootnotes, pObjectsOnPage);
                                        colHTML.AppendFormat(@"{0}<br>", antimicrobialHTML);
                                    }
                                    html.Append(colHTML);
                                }
                    ...
                    case BDConstants.LayoutVariantType.PregnancyLactation_Antimicrobials_Pregnancy:
                        ...
                        html.Append(BuildBDCategoryPLAntimicrobialsInPregnancyHtml(pContext, pNode, pFootnotes, pObjectsOnPage, pLevel));
                        break;
                    case BDConstants.LayoutVariantType.PregnancyLactation_Antimicrobials_Lactation:
                        // high complexity: this is an exception method
                        html.Append(BuildBDCategoryPLAntimicrobialsInLactationHtml(pContext, pNode, pFootnotes, pObjectsOnPage, pLevel));
                        break;
        ...
        public string BuildBDCategoryPLAntimicrobialsInLactationHtml(Entities pContext, IBDNode pNode, List<BDLinkedNote> pFootnotes, List<Guid> pObjectsOnPage, int pLevel)
        {
            ...
            if ((null != pNode) && (pNode.NodeType == BDConstants.BDNodeType.BDCategory))
            {
                ...
                string c2Html = buildHtmlForMetadataColumn(pContext, pNode, metadataLayoutColumns[1], BDConstants.BDNodeType.BDAntimicrobialRisk, BDAntimicrobialRisk.PROPERTYNAME_LACTATIONRISK, pFootnotes, pObjectsOnPage);
                string c3Html = buildHtmlForMetadataColumn(pContext, pNode, metadataLayoutColumns[2], BDConstants.BDNodeType.BDAntimicrobialRisk, BDAntimicrobialRisk.PROPERTYNAME_APPRATING, pFootnotes, pObjectsOnPage);
                string c4Html = buildHtmlForMetadataColumn(pContext, pNode, metadataLayoutColumns[3], BDConstants.BDNodeType.BDAntimicrobialRisk, BDAntimicrobialRisk.PROPERTYNAME_RELATIVEDOSE, pFootnotes, pObjectsOnPage);

                // begin new page (with a table of six columns) on Category
                html.AppendFormat(@"<table class=""v{0}""><tr><th colspan=3>{1}</th><th>{2}</th><th>{3}</th><th>{4}</th></tr>", pNode.LayoutVariant, c1Html, c2Html, c3Html, c4Html);
                List<IBDNode> childNodes = BDFabrik.GetChildrenForParent(pContext, pNode);
                foreach (IBDNode subcategory in childNodes)
                {
                    // subcategory is a grouping of drugs:  write a row
                    html.AppendFormat("<tr><td colspan = 3>{0}</td><td /><td /><td /></tr>", buildNodeWithReferenceAndOverviewHTML(pContext, subcategory, "b", pFootnotes, pObjectsOnPage));
                    List<IBDNode> subcategoryChildNodes = BDFabrik.GetChildrenForParent(pContext, subcategory);

                    //the next layer can either be antimicrobial group or antimicrobial
                    foreach (IBDNode node in subcategoryChildNodes)
                    {
                        if (node.NodeType == BDConstants.BDNodeType.BDAntimicrobialGroup)
                        {
                            // write to column 2
                            html.AppendFormat("<tr><td /><td colspan=2>{0}</td><td /><td /><td /></tr>", buildNodeWithReferenceAndOverviewHTML(pContext, node, "u", pFootnotes, pObjectsOnPage));
                            List<IBDNode> antimicrobials = BDFabrik.GetChildrenForParent(pContext, node);
                            foreach (IBDNode antimicrobial in antimicrobials)
                            {
                                // begin detail table row
                                html.AppendFormat("<tr><td /><td /><td>{0}</td>", buildNodeWithReferenceAndOverviewHTML(pContext, antimicrobial as BDNode, string.Empty, pFootnotes, pObjectsOnPage));
                                List<IBDNode> antimicrobialRisks = BDFabrik.GetChildrenForParent(pContext, antimicrobial);
                                foreach (IBDNode antimicrobialRisk in antimicrobialRisks)
                                {
                                    BDAntimicrobialRisk amRisk = antimicrobialRisk as BDAntimicrobialRisk;
                                    html.AppendFormat("<td>{0}</td><td>{1}</td><td>{2}</td></tr>", amRisk.riskFactor, amRisk.aapRating, amRisk.relativeInfantDose);
                                }
                            }
                        }
                        else
                        {
                            // write to column 3
                            html.AppendFormat("<tr><td /><td /><td>{0}</td>", buildNodeWithReferenceAndOverviewHTML(pContext, node as BDNode, string.Empty, pFootnotes, pObjectsOnPage));
                            List<IBDNode> antimicrobialRisks = BDFabrik.GetChildrenForParent(pContext, node);
                            foreach (IBDNode antimicrobialRisk in antimicrobialRisks)
                            {
                                BDAntimicrobialRisk amRisk = antimicrobialRisk as BDAntimicrobialRisk;
                                html.AppendFormat("<td>{0}</td><td>{1}</td><td>{2}</td></tr>", amRisk.riskFactor, amRisk.aapRating, amRisk.relativeInfantDose);
        ...
        public string BuildBDCategoryPLAntimicrobialsInPregnancyHtml(Entities pContext, IBDNode pNode, List<BDLinkedNote> pFootnotes, List<Guid> pObjectsOnPage, int pLevel)
        {
            ...
            if ((null != pNode) && (pNode.NodeType == BDConstants.BDNodeType.BDCategory))
            {
                ...
                string c1Html = buildHtmlForMetadataColumn(pContext, pNode, metadataLayoutColumns[0], BDConstants.BDNodeType.BDAntimicrobial, BDNode.PROPERTYNAME_NAME, pFootnotes, pObjectsOnPage);
                string c2Html = buildHtmlForMetadataColumn(pContext, pNode, metadataLayoutColumns[1], BDConstants.BDNodeType.BDAntimicrobialRisk, BDAntimicrobialRisk.PROPERTYNAME_PREGNANCYRISK, pFootnotes, pObjectsOnPage);
                string c3Html = buildHtmlForMetadataColumn(pContext, pNode, metadataLayoutColumns[2], BDConstants.BDNodeType.BDAntimicrobialRisk, BDAntimicrobialRisk.PROPERTYNAME_RECOMMENDATION, pFootnotes, pObjectsOnPage);

                // begin new page (with a table of four columns) on Category
                html.AppendFormat(@"<table class=""v{0}""><tr><th colspan=2>{1}</th><th>{2}</th><th>{3}</th></tr>", pNode.LayoutVariant, c1Html, c2Html, c3Html);

                foreach (IBDNode subcategory in categoryChildren)
                {
                    // subcategory is a grouping of drugs: write a new row
                    html.AppendFormat("<tr><td colspan = 2><b>{0}</b></td><td /><td /></tr>", buildNodeWithReferenceAndOverviewHTML(pContext, subcategory as BDNode, "b", pFootnotes, pObjectsOnPage));
                    pObjectsOnPage.Add(subcategory.Uuid);
                    List<IBDNode> antimicrobials = BDFabrik.GetChildrenForParent(pContext, subcategory);
                    foreach (IBDNode antimicrobial in antimicrobials)
                    {
                        // write a row to the table, leaving the first column blank.
                        html.AppendFormat("<tr><td /><td>{0}</td>", buildNodeWithReferenceAndOverviewHTML(pContext, antimicrobial as BDNode, string.Empty, pFootnotes, pObjectsOnPage));
                        ...
                        {
                            BDAntimicrobialRisk risk = amRisk as BDAntimicrobialRisk;
                            html.AppendFormat("<td>{0}</td>", buildNodePropertyHTML(pContext, risk, risk.riskFactor, BDAntimicrobialRisk.PROPERTYNAME_PREGNANCYRISK, false, pFootnotes, pObjectsOnPage));
                            html.AppendFormat("<td>{0}</td>", buildNodePropertyHTML(pContext, risk, risk.recommendations, BDAntimicrobialRisk.PROPERTYNAME_RECOMMENDATION, false, pFootnotes, pObjectsOnPage));
                            pObjectsOnPage.Add(amRisk.Uuid);
                        }
                        html.Append("</tr>");
                    }
                }
...
//            childDefinitionList.Add(new Tuple<BDConstants.BDNodeType, BDConstants.LayoutVariantType[]>(BDConstants.BDNodeType.BDMicroorganismGroup, new BDConstants.LayoutVariantType[] { BDConstants.LayoutVariantType.Prophylaxis_IEDrugAndDosage }));
                ...
                foreach (IBDNode mGroup in mGroups)
                {
                    ...
                    foreach (IBDNode microorganism in microorganisms)
                    {
                        ...
                        foreach (IBDNode precaution in precautions)
                        {
                            ...
                            // build table
                            mHTML.AppendFormat("<table><tr><th>Precautions{0}</th><th>Acute Care</th><th>Long Term Care</th></tr>",
                                buildFooterMarkerForList(retrieveNotesForParentAndPropertyOfLinkedNoteType(pContext, precaution.Uuid, BDPrecaution.PROPERTYNAME_ORGANISM_1, BDConstants.LinkedNoteType.Footnote), true, mFootnotes, mObjectsOnPage)); //ks: footnote & objOnPg changed from pX to mX
                            mHTML.AppendFormat("<tr><td>Single Room</td><td>{0}</td><td>{1}</td></tr>", p.singleRoomAcute, p.singleRoomLongTerm);
                            mHTML.AppendFormat("<tr><td>Gloves</td><td>{0}</td><td>{1}</td></tr>", p.glovesAcute, p.glovesLongTerm);
                            mHTML.AppendFormat("<tr><td>Gowns</td><td>{0}</td><td>{1}</td></tr>", p.gownsAcute, p.gownsLongTerm);
                            mHTML.AppendFormat("<tr><td>Mask</td><td>{0}</td><td>{1}</td></tr>", p.maskAcute, p.maskLongTerm);
                ...
                foreach (IBDNode child in childNodes)
                {
                    switch (child.LayoutVariant)
                    {
                        ...
                        default:
                            ...
                            List<BDLinkedNote> therapyFootnotes = new List<BDLinkedNote>();
                            List<IBDNode> therapies = BDFabrik.GetChildrenForParent(pContext, child);
                            therapyHTML.Append(buildNodeWithReferenceAndOverviewHTML(pContext, child, HtmlHeaderTagLevelString(pLevel + 1), therapyFootnotes, therapiesOnPage));
                            string subtext = "given 30-60 minutes before the procedure";
                            therapyHTML.AppendFormat("<table><tr><th>{0}</th><th>{1} {2}/ROUTE</th><th>{3} {4}/ROUTE</th></tr>",
                                c2Html, c3Html, subtext, c4Html, subtext);
                            foreach (IBDNode t in therapies)
            ...
            if ((null != pNode) && (pNode.NodeType == BDConstants.BDNodeType.BDTopic))
            {
                ...
                switch (pNode.LayoutVariant)
                {
                    ...
                    case BDConstants.LayoutVariantType.TreatmentRecommendation01_Gastroenteritis_CultureDirected:
                        foreach (IBDNode child in children)
                        {
                            html.Append(BuildBDPathogenGroupHtml(pContext, child, pFootnotes, pObjectsOnPage, pLevel + 1));
                        }
                        break;
                    case BDConstants.LayoutVariantType.TreatmentRecommendation10_Fungal:
                        //childDefinitionList.Add(new Tuple<BDConstants.BDNodeType, BDConstants.LayoutVariantType[]>(BDConstants.BDNodeType.BDAttachment, new BDConstants.LayoutVariantType[] { layoutVariant }));
                ...
                switch (pNode.LayoutVariant)
                {
                    case BDConstants.LayoutVariantType.TreatmentRecommendation13_VesicularLesions:
                        ...
                        foreach (IBDNode frequency in children)
                        {
                            ...
                            foreach (IBDNode pathogenGroup in pathogenGroups)
                            {
                                html.Append(BuildBDPathogenGroupHtml(pContext, pathogenGroup as BDNode, pFootnotes, pObjectsOnPage, pLevel + 1));
                            }
                        }
                        break;
                    default:
                ...
                foreach (IBDNode child in children)
                {
                    switch (child.NodeType)
                    {
                        ...
                        case BDConstants.BDNodeType.BDTopic:
                            ...
                            break;
                        default:
                            string errorMsg = string.Format("buildBDPresentationHtml : Unhandled Presentation child node type [{0}]", child.NodeType.ToString());
        ...
        private string buildTherapyHtml(Entities pContext, BDTherapy pTherapy, List<BDLinkedNote> pFootnotes, List<Guid> pObjectsOnPage)
        {
            ...
            if (pTherapy.nameSameAsPrevious.Value == true)
                therapyNameHtml = buildNodePropertyHTML(pContext, pTherapy, previousTherapyId, previousTherapyName, BDTherapy.PROPERTYNAME_THERAPY, false, pFootnotes, pObjectsOnPage);
            else
                therapyNameHtml = buildNodePropertyHTML(pContext, pTherapy, pTherapy.Uuid, pTherapy.Name, BDTherapy.PROPERTYNAME_THERAPY, false, pFootnotes, pObjectsOnPage);

            if(!string.IsNullOrEmpty(therapyNameHtml))
                therapyHtml.AppendFormat("<b>{0}</b>", therapyNameHtml);

            if (pTherapy.rightBracket.Value == true)
                therapyHtml.Append(@"&nbsp&#93");
            ...
            if (pTherapy.dosageSameAsPrevious.Value == true)
                dosageHtml = buildNodePropertyHTML(pContext, pTherapy, previousTherapyId, previousTherapyDosage, BDTherapy.PROPERTYNAME_DOSAGE, false, pFootnotes, pObjectsOnPage);
            else
                dosageHtml = buildNodePropertyHTML(pContext, pTherapy, pTherapy.Uuid, pTherapy.dosage, BDTherapy.PROPERTYNAME_DOSAGE, false, pFootnotes, pObjectsOnPage);
            ...
            if (pTherapy.durationSameAsPrevious.Value == true)
                durationHtml = buildNodePropertyHTML(pContext, pTherapy, previousTherapyId, previousTherapyDuration, BDTherapy.PROPERTYNAME_DURATION, false, pFootnotes, pObjectsOnPage);
            else
                durationHtml = buildNodePropertyHTML(pContext, pTherapy, pTherapy.Uuid, pTherapy.duration, BDTherapy.PROPERTYNAME_DURATION, false, pFootnotes, pObjectsOnPage);
        ...
        public string buildTherapyWithTwoDurationsHtml(Entities pContext, BDTherapy pTherapy, List<BDLinkedNote> pFootnotes, List<Guid> pObjectsOnPage)
        {
            ...
            if (pTherapy.nameSameAsPrevious.Value == true)
                therapyHtml.AppendFormat("{0}", buildNodePropertyHTML(pContext, pTherapy, previousTherapyId, previousTherapyName, BDTherapy.PROPERTYNAME_THERAPY, false, "b", pFootnotes, pObjectsOnPage));
            else
                therapyHtml.AppendFormat("{0}", buildNodePropertyHTML(pContext, pTherapy, pTherapy.Uuid, pTherapy.Name, BDTherapy.PROPERTYNAME_THERAPY, false, "b", pFootnotes, pObjectsOnPage));
        ...
        private string buildNodeWithReferenceAndOverviewHTML(Entities pContext, IBDNode pNode, string pHeaderTagLevel, List<BDLinkedNote> pFootnotes, List<Guid> pObjectsOnPage)
        {
            ...
            nodeHTML.Append(buildNodePropertyHTML(pContext, pNode, pNode.Uuid, pNode.Name, BDNode.PROPERTYNAME_NAME, false, pHeaderTagLevel, pFootnotes, pObjectsOnPage));
        ...
        private string buildTableRowHtml(Entities pContext, BDTableRow pRow, bool forceHeaderRow, bool markFooterAtEnd, List<BDLinkedNote> pFootnotes, List<Guid> pObjectsOnPage)
        {
            ...
            if (pRow != null)
            {
                ...
                for (int i = 0; i < cells.Count; i++)
                {
                    ...
                    string cellHTML = buildNodePropertyHTML(pContext, tableCell, tableCell.Uuid, tableCell.value, BDTableCell.PROPERTYNAME_CONTENTS, false, itemFooters, pObjectsOnPage);
                    ...
                    else
                    {
                       string itemFooterMarker = buildFooterMarkerForList(itemFooters, true, pFootnotes, pObjectsOnPage);
                        ...
                        if (markFooterAtEnd)
                            tableRowHTML.AppendFormat(@"{0}{1}{2}{3}", startTag, cellHTML, itemFooterMarker, endCellTag);
                        else
                        {
                            int lineBreakIndex = tableCell.value.Length;
                            if (tableCell.value.Contains("\n"))
                                lineBreakIndex = tableCell.value.IndexOf("\n");
                            string cellTextWithFooterTag = cellHTML.Insert(lineBreakIndex, itemFooterMarker);
        ...
        private string buildAntibioticsCSFPenetrationDosagesHTML(Entities pContext, IBDNode pNode, List<BDLinkedNote> pFootnotesOnPage, List<Guid> pObjectsOnPage)
        {
            ...
            if (childNodes.Count > 0)
            {
                ...
                {
                    BDConfiguredEntry entry = child as BDConfiguredEntry;
                    string ce1HTML = buildNodePropertyHTML(pContext, child, child.Uuid, child.Name, BDConfiguredEntry.PROPERTYNAME_NAME, false, pFootnotesOnPage, pObjectsOnPage);
                    string ce2HTML = buildNodePropertyHTML(pContext, child, child.Uuid, entry.field01, BDConfiguredEntry.PROPERTYNAME_FIELD01, false, pFootnotesOnPage, pObjectsOnPage);
                    string ce3HTML = buildNodePropertyHTML(pContext, child, child.Uuid, entry.field02, BDConfiguredEntry.PROPERTYNAME_FIELD02, false, pFootnotesOnPage, pObjectsOnPage);
        ...
        private List<BDHtmlPage> buildAntimicrobialWithRiskHTML(Entities pContext, List<string> columnHtml, List<IBDNode> antimicrobials)
        {
            ...
            foreach (IBDNode antimicrobial in antimicrobials)
            {
                ...
                {
                    BDAntimicrobialRisk risk = amRisk as BDAntimicrobialRisk;
                    antimicrobialHTMLBody.AppendFormat("<b>{0}</b>: {1}", columnHtml[0], risk.riskFactor);
                    antimicrobialHTMLBody.AppendFormat("<p><b>{0}</b>: {1}</p>", columnHtml[1], risk.aapRating);
                    antimicrobialHTMLBody.AppendFormat("<p><b>{0}</b>: {1}</p>", columnHtml[2], risk.relativeInfantDose);
                    antimicrobialsOnPage.Add(amRisk.Uuid);
                }
        ...
        /// <param name="pObjectsOnPage"></param>
        /// <returns></returns>
        private string buildHtmlForMetadataColumn(Entities pContext, IBDNode pPageDisplayParent, BDLayoutMetadataColumn pColumn, BDConstants.BDNodeType pNodeType, string pPropertyName, List<BDLinkedNote> pFootnotes, List<Guid> pObjectsOnPage)
        {
            StringBuilder columnHtml = new StringBuilder();
            string cLabel = retrieveMetadataLabelForPropertyName(pContext, pNodeType, pPropertyName, pColumn);
            ...
            List<BDLinkedNote> cFootnotes = retrieveNotesForLayoutColumn(pContext, pColumn, BDConstants.LinkedNoteType.Footnote);
            ...
            BDHtmlPage notePage = generatePageForLinkedNotesLayoutColumn(pContext, pColumn, marked, unmarked);
            //BDHtmlPage notePage = generatePageForLinkedNotes(pContext, pColumn.Uuid, pPageDisplayParent.NodeType, null, marked, unmarked, pObjectsOnPage);
        ...
        /// <param name="pObjectsOnPage"></param>
        /// <returns></returns>
        private string buildNodePropertyHTML(Entities pContext, IBDNode pNode, Guid pNoteParentId, string pPropertyValue, string pPropertyName, bool pShowNotesInline, string pHtmlTag, List<BDLinkedNote> pFootnotes, List<Guid> pObjectsOnPage)
        {
